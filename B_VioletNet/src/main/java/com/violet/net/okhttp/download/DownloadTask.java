 ...
 */
package com.violet.net.okhttp.download;
...
import com.violet.net.http.server.download.DownloadListener;
import com.violet.net.http.server.download.IDownloadTask;
import com.violet.net.okhttp.OkDownload;
...
import com.violet.net.okhttp.exception.OkGoException;
import com.violet.net.okhttp.exception.StorageException;
 ...
 * ================================================
 */
public class DownloadTask extends AbsDownloadTask<DownloadTask> {
...
//    private PriorityRunnable priorityRunnable;
    ...
    public DownloadTask(String tag, Request request) {
...
//        executor = OkDownload.getInstance().getThreadPool().getExecutor();
    ...
    public DownloadTask save() {
        ...
        DownloadDBManager.getInstance().replace(progress);
        return this;
    }
    ...
    @Override
    public void start() {
        if (OkDownload.getInstance().getTask(progress.tag) == null || DownloadDBManager.getInstance().get(progress.tag) == null) {
...
//            executor.execute(priorityRunnable);
    ...
    private void startInternal(){
        HttpUtils.runOnSubThread(new Runnable() {
            ...
            public void run() {
                //check breakpoint
                final long startPosition = progress.currentSize;
                if (startPosition < 0) {
                ...
                progress.request.headers(HttpHeaders.HEAD_KEY_RANGE, "bytes=" + startPosition + "-");
                progress.request.converter(new JsonConvert(Response.class));
                HttpManager.getInstance().execute(progress.request, new CommonCallback<Response>() {


                    @Override
                    public void onError(com.violet.net.http.model.Response<Response> response) {
                        Throwable e = response == null? null : response.getException();
                        if(e == null){
                            e = new HttpException("network error!");
                        }
                        postOnError(progress, e);
                    }

                    @Override
                    public void onSuccess(com.violet.net.http.model.Response<Response> responseWrapper) {
                        Response response = responseWrapper.body();
                        if(response == null){
                            return;
                        }
...
//        executor.remove(priorityRunnable);
        //progress.request.getCall().cancel();//TODO
    ...
    public DownloadTask remove(boolean isDeleteFile) {
        ...
        DownloadDBManager.getInstance().delete(progress.tag);
        IDownloadTask task = OkDownload.getInstance().removeTask(progress.tag);
    ...
    private void updateDatabase(Progress progress) {
        ...
        DownloadDBManager.getInstance().update(contentValues, progress.tag);
    }
}
