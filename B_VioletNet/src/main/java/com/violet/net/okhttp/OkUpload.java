...
import com.violet.net.okhttp.upload.UploadTask;
...
public class OkUpload {
    ...
    private Map<String, AbsUploadTask<?>> taskMap;         //所有任务
...
//        threadPool = new UploadThreadPool();
        ...
        List<Progress> taskList = UploadDBManager.getInstance().getUploading();
        ...
        UploadDBManager.getInstance().replace(taskList);
    }

    public static <T> UploadTask<T> request(String tag, Request request) {
        Map<String, AbsUploadTask<?>> taskMap = OkUpload.getInstance().getTaskMap();
    ...
    /** 从数据库中恢复任务 */
    public static List<AbsUploadTask<?>> restore(List<Progress> progressList) {
        Map<String, AbsUploadTask<?>> taskMap = OkUpload.getInstance().getTaskMap();
        List<AbsUploadTask<?>> tasks = new ArrayList<>();
        for (Progress progress : progressList) {
            AbsUploadTask<?> task = taskMap.get(progress.tag);
            if (task == null) {
                task = new UploadTask<>(progress);
                taskMap.put(progress.tag, task);
            }
    ...
    public void startAll() {
        for (Map.Entry<String, AbsUploadTask<?>> entry : taskMap.entrySet()) {
            AbsUploadTask<?> task = entry.getValue();
            if (task == null) {
    ...
    public void removeAll() {
        Map<String, AbsUploadTask<?>> map = new HashMap<>(taskMap);
        //先删除未开始的任务
        for (Map.Entry<String, AbsUploadTask<?>> entry : map.entrySet()) {
            AbsUploadTask<?> task = entry.getValue();
            if (task == null) {
                OkLogger.w("can't find task with tag = " + entry.getKey());
                continue;
...
//    }
    ...
    public AbsUploadTask<?> getTask(String tag) {
    ...
    public AbsUploadTask<?> removeTask(String tag) {
