...
import com.violet.net.okhttp.exception.CacheException;
...
//            runOnCallerOrSubThread(run);
    ...
    @Override
    public boolean onAnalysisResponse(final VtCall call, final okhttp3.Response response) {
        if (response.code() != 304) return false;

        if (cacheEntity == null) {
            final Response error = OkHttpUtil.error(true, request, response.headers(), CacheException.NON_AND_304(request.getCacheKey()),response.code());
            Runnable run = new Runnable() {
                @Override
                public void run() {
                    mCallback.onError(error);
...
//                runOnCallerOrSubThread(run);
            ...
            final Response success = OkHttpUtil.success2(true, cacheEntity.getData(), request, response.headers(),response.code());
            Runnable run = new Runnable() {
                @Override
                public void run() {
...
//                runOnCallerOrSubThread(run);
    ...
    public Response requestSync(CacheEntity cacheEntity) {
        ...
        if (response.isSuccessful() && response.code() == 304) {
            if (cacheEntity == null) {
                response = OkHttpUtil.error2(true, request, response.getHeaders(), CacheException.NON_AND_304(request.getCacheKey()), response.code());
            } else {
                response = OkHttpUtil.success2(true, cacheEntity.getData(), request, response.getHeaders(), response.code());
            }
        }
        return response;
    }
