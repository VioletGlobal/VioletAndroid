...
public class CarouselLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    /**
     * scrollBy接管RecyclerView的滑动方法, 在滑动时重新对RecyclerView进行计算位置并布局
     * <p>
    ...
    /**
     * @param recycler             Rv 缓存类
     * @param width                Rv的宽度
     * @param height               Rv的高度
     * @param childMeasuringNeeded 是否需要measure子View
     */
    private void fillDataHorizontal(final RecyclerView.Recycler recycler, final int width, final int height, final boolean childMeasuringNeeded) {
        ...
        // 根据LayoutOrder进行布局,必须保证最后布局正中间的View
        for (int i = 0, count = mLayoutHelper.mLayoutOrder.length; i < count; ++i) {
            // 遍历LayoutOrder
            final LayoutOrder layoutOrder = mLayoutHelper.mLayoutOrder[i];
            // 获取每个Card相对于别的Card的偏移量
            final int offset = getCardOffsetByPositionDiff(layoutOrder.mItemPositionDiff);
     ...
     * @param recycler
     * @param i
     * @param childMeasuringNeeded
     */
    @SuppressWarnings("MethodWithTooManyParameters")
    private void fillChildItem(final int start, final int top, final int end,
                               final int bottom, @NonNull final LayoutOrder layoutOrder,
                               @NonNull final RecyclerView.Recycler recycler,
                               int i, final boolean childMeasuringNeeded) {
        // 从Recycler容器中获取缓存的View
        final View view = bindChild(layoutOrder.mItemAdapterPosition, recycler, childMeasuringNeeded);
        // 根据LayoutOrder的位置，设置层级关系，设置View的层级关系
        setViewElevation(getLayerIndex(i), view);
        ItemTransformation transformation = null;
        if (null != mViewPostLayout) {
            transformation = mViewPostLayout.transformChild(view, layoutOrder.mItemPositionDiff, mOrientation, i, getLayoutCount(), layoutOrder.mItemAdapterPosition);
        }
    ...
    protected void setViewElevation(int layerIndex, View view) {
        ViewCompat.setElevation(view, layerIndex);
    }
    ...
    private void generateLayoutOrder(final float currentScrollPosition, @NonNull final RecyclerView.State state) {
        ...
        if (mCircleLayout && 1 < mItemsCount) {
            ...
            // 需要布局的View个数,这里是最少两个层叠的View和下面的View(仅露出一点), 一个顶View,
            int layoutCount = getLayoutCount(mItemsCount);// + 3 = 1 (center item) + 2 (addition bellow maxVisibleItems)
            ...
            for (int i = layoutCount - 1; i >= countLayoutHalf + 1; --i) {
                ...
                // currentPosition 0, adapterPosition 1,2,3
                // arrayPosition 5,4,3
    ...
    protected int getCardOffsetByPositionDiff(final float itemPositionDiff) {
        ...
        if (VERTICAL == mOrientation) {
            ...
            //  fixme 偏移量的hardcode, 每个控件的偏移相对其他控件的偏移量
     ...
     *
     * @param itemPositionDiff current item difference with layout center line. if this is 0,
     *                         then this item center is in layout center line.
     *                         if this is 1 then this item is bellow the layout center line in the full item size distance.
     ...
     */
    // 将当前View滑动的位置调整到adapter中的正常范围,只在使用循环布局时使用
    private static float makeScrollPositionInRange0ToCount(final float currentScrollPosition, final int count) {
        ...
        while (absCurrentScrollPosition < 0) {
    ...
    public int getLayoutCount(){
        return mLayoutHelper.mLayoutOrder.length;
    }
    ...
    public interface PostLayoutListener {
         ...
         * @param index                    layout order in LayoutOrders
         * @param totalCount               total layout count
        ...
        ItemTransformation transformChild(@NonNull final View child, final float itemPositionToCenterDiff
                , final int orientation, final int index, final int totalCount, int adapterPosition);
    }
    ...
    private static class LayoutHelper {
        ...
        // 复用LayoutOrder对象,相当于对象池,减少重复创建辅助对象的产生的性能消耗
